...
Java_com_gacrnd_gcs_binder_MainActivity_writeTest(JNIEnv *env, jobject thiz) {
    __android_log_print(ANDROID_LOG_ERROR, "jack", "==========================");
...
//        System.arraycopy(src, 0, dst, 0, src.length);
    ...
    __android_log_print(ANDROID_LOG_ERROR, "jack", "写入数据:%s", data.c_str());
}

extern "C"
JNIEXPORT void JNICALL
Java_com_gacrnd_gcs_binder_MainActivity_readTest(JNIEnv *env, jobject thiz) {
    //申请内存
    ...
    __android_log_print(ANDROID_LOG_ERROR, "jack", "读取数据:%s", result.c_str());
