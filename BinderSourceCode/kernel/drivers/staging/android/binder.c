					   ...
					   binder_uintptr_t cookie)
{
	...
	INIT_LIST_HEAD(&node->work.entry);  //创建  work  和 todo --》类似 messageQueue
			...
			binder_size_t *consumed)
{
	...
	while (ptr < end && thread->return_error == BR_OK) {
		...
		case BC_ENTER_LOOPER:
			...
			thread->looper |= BINDER_LOOPER_STATE_ENTERED;  //设置线程为BINDER_LOOPER_STATE_ENTERED状态
			break;
		case BC_EXIT_LOOPER:
			binder_debug(BINDER_DEBUG_THREADS,
				     "%d:%d BC_EXIT_LOOPER\n",
			      ...
			      binder_size_t *consumed, int non_block)
{
	...
	//binder_looper等待的时候，consumed为0，会将bwr命令放入BR_NOOP
	if (*consumed == 0) {
...
retry:
	//wait_for_proc_work 由于todo为空，wait_for_proc_work是true
